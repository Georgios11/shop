...
import { paginate } from '../util/paginationUtils';
...
export const getAllUsers = async (
  ...
  try {
    ...
    // const keyword = createKeywordFilter<ProductDocument>(req);

    // const { documents, startIndex, endIndex, count, availablePages } =
    //   await paginate(pageNumber, User, 'users', keyword);

    // responseUtils.successResponse(
    //   res,
    //   StatusCodes.OK,
    //   `Returned users ${startIndex}-${endIndex} of ${count}`,
    //   { users: documents, pageNumber, pages: availablePages }
    // );
 ...
 *     description: Retrieves a paginated list of all orders. Only accessible to admins.
 *     tags:
 *       - Admin - Requires administrator privileges
